...
import static orego.neural.Rich1Classifier.BIAS;
...
public class Rich1Player extends McPlayer {
	/**
	...
	protected int bestSearchMove(Board board, Rich1Classifier classifier) {
		int bestMove = PASS;
	...
	public void generateMovesToFrontier(McRunnable runnable) {
		...
		int count = 0;
		Rich1Classifier classifier = ((Rich1McRunnable) runnable)
				.getClassifier();
	...
	public double getWinRate(int p) {
		// TODO maybe be better to Rich1 over all of the McRunnables
		Rich1Classifier classifier = ((Rich1McRunnable) getRunnable(0))
				.getClassifier();
	...
	protected String goguiPrimaryVariation() {
		...
		for (int depth = 0; depth < cutoff; depth++) {
			int best = bestSearchMove(board,
					((Rich1McRunnable) getRunnable(0)).getClassifier());
	...
	public void incorporateRun(int winner, McRunnable runnable) {
		if (winner != VACANT) {
			...
			// double[][][][] weights = classifier.getWeights();
			// System.out
			// .println(weights[BLACK][Rich1Classifier.BIAS][0][at("d5")]
			// + "\t"
...
//					+ "\t" + classifier.evaluate(getBoard().getColorToPlay(), at("a9"), getBoard(), getTurn()));				
	...
	public void reset() {
		super.reset();
		for (int i = 0; i < getNumberOfThreads(); i++) {
			setRunnable(i, new Rich1McRunnable(this, getPolicy().clone(),
					getLearn(), getHistory()));
		}
