...
# Adapted (bug fix and additional result printing) by Peter Drake
# Search for "Drake" to see changes
...
import string
import re
from systemconfig import * # Drake
...
class GTP_connection:
    ...
    def exec_cmd(self, cmd):
        ...
        if len(result) == 0:
            return "ERROR: len = 0"
        ### Moved by Peter Drake, 23 Jan 2008
        ### This used to be BEFORE checking len(result)
...
class GTP_game:
    ...
    def writesgf(self, sgffilename):
        ...
        size = self.size
        outfile = open(RESULT_DIR + "/" + sgffilename, "w") # Drake
        if not outfile:
            ...
            if to_play == "B":
                ...
                if move[:6] == "resign":
                    ...
                    if string.lower(move[:4]) == "pass":
                        passes = passes + 1
                        if verbose >= 1:
                            print "Black passes" # Was White; fixed by Drake
                    else:
                        passes = 0
                ...
                if move[:6] == "resign":
                    ...
                    if string.lower(move[:4]) == "pass":
                        passes = passes + 1
                        if verbose >= 1:
                            print "White passes" # Was Black; fixed by Drake
                    else:
                        passes = 0
...
if endgame_filelist != []:
    ...
    print "Black %d, white %d" % (win_black, win_white) # Drake
    if (cputimes["white"] != "0"):
