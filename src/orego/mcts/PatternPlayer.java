...
//			result = goguiPatternWinRates((command.charAt(command.length()-1)-'0')/2-1);
		} else {
		...
		for (int p: getAllPointsOnBoard()){
			if (getBoard().getColor(p) == VACANT){
				...
				if (winRate>0){
					if (result.length()>0)
						result += '\n';
					PatternInformation info = getInformation(patternType,getBoard().getPatternHash(patternType, p));
		...
		for (int p: getAllPointsOnBoard()){
			if (getBoard().getColor(p) == VACANT){
				...
				if (winRate>0){
					if (result.length()>0)
						result += '\n';
					result += String.format("LABEL %s %.0f%%",
							pointToString(p), getInformation(patternType, getBoard().getPatternHash(patternType, p)).getRuns());
				}
			}
		}
		return result;
