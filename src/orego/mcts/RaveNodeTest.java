...
public class RaveNodeTest {
	...
	public void testRecordPlayout() {
		node.recordPlayout(1, new int[] {at("a1"), at("b2"), at("a2")}, 0, 3, new IntSet(FIRST_POINT_BEYOND_BOARD));
		assertEquals(3, node.getRaveRuns(at("a1")), 0.001);
		assertEquals(2, node.getRaveWins(at("a1")), 0.001);
	...
	public void testRecordTiedPlayout() {
		node.recordPlayout((float) 0.5, new int[] {at("a1"), at("b2"), at("a2")}, 0, 3, new IntSet(FIRST_POINT_BEYOND_BOARD));
		assertEquals(3, node.getRaveRuns(at("a1")), 0.001);
