...
//			force.add(p);
//		}
		int[] fwins = new int[getFirstPointBeyondBoard()];
		int[] fruns = new int[getFirstPointBeyondBoard()];
...
//					if (force.contains(moves.get(i)[k - 1])) {
		...
		double winrate[] = new double[getFirstPointBeyondBoard()];
	...
	public double[] computeWinrates() {
		int[] wins = new int[getFirstPointBeyondBoard()];
		int[] runs = new int[getFirstPointBeyondBoard()];
		for (int i = 0; i < turn.size(); i++) { // For each playout
			int win = result.get(i);
