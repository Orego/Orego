...
public class FilterPlayer extends McPlayer {
	...
	/** Pattern multistage filter. */
	private MultiStageFilter patterns;
	...
	public FilterPlayer(boolean maintainHashes) {
...
//		result.add("gogui-primary-variation");
		result.add("gogui-reasonable-moves");
		result.add("gogui-reasonable-moves-radius");
		return result;
	}
...
//		result.add("gfx/Primary variation/gogui-primary-variation");
		result.add("gfx/Reasonable moves/gogui-reasonable-moves");// %s");
		result.add("gfx/Reasonable moves at radius/gogui-reasonable-moves-radius %s");
		return result;
	}
...
//	}
	...
	protected String goguiReasonableMoves() {
		String result = "";
		for (int p : getAllPointsOnBoard()) {
			if (getBoard().getColor(p) == VACANT) {
				if (result.length() > 0)
	...
	protected String goguiReasonableMoves(int radius) {
		String result = "";
		for (int p : getAllPointsOnBoard()) {
			if (getBoard().getColor(p) == VACANT) {
				if (result.length() > 0)
					result += '\n';
				int reasonable = 0;
				if (patterns.isReasonable(getBoard(), p,radius)){
					reasonable = 1;
				}
				// Number of playouts through each point
				result += String.format("COLOR %s %s\nLABEL %s %d",
...
//		} else 
		if (command.equals("gogui-reasonable-moves")) {
//			patterns.setThreshold(Integer.parseInt(arguments.nextToken()));
			result = goguiReasonableMoves();
		} else if (command.equals("gogui-reasonable-moves-radius")) {
			result = goguiReasonableMoves(Integer.parseInt(arguments.nextToken()));
...
//		debug("Noise: " + noise);
	...
	protected void loadPatternMultiStageFilter() {
		try {
