...
public class SearchNodeTest {
	...
	public void testIsFresh() {
		...
		node.recordPlayout(1, new int[] { PASS }, 0, 1, new ShortSet(
				coords.getFirstPointBeyondBoard()));
	}

	@Test
	public void testToString() {
		node.recordPlayout(1, new int[] { at("a1") }, 0, 1, new ShortSet(
				coords.getFirstPointBeyondBoard()));
		node.recordPlayout(1, new int[] { PASS }, 0, 1, new ShortSet(
				coords.getFirstPointBeyondBoard()));
	...
	public void testGetWinningMove() {
		...
		node.recordPlayout(0, new int[] { at("a1") }, 0, 1, new ShortSet(
				coords.getFirstPointBeyondBoard()));
		assertEquals(NO_POINT, node.getWinningMove());
	}
	...
	public void testTieUpdate() {
		node.reset(0L, coords);
		node.recordPlayout((float) 0.5, new int[] { at("a1") }, 0, 1, new ShortSet(
				coords.getFirstPointBeyondBoard()));
